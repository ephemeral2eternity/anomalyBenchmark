        ...
        try:
            if "ico" in self.command:
                ...
                with open(script_folder + "/anomaly.log", "a") as logFile:
                    logFile.write(
                        str(time.time()) + ", cpu, " + str(cpu_stress_period) + ", " + str(cpu_stress_workers) + "\n")

                p = sub.Popen('stress --cpu ' + cpu_stress_workers + ' --timeout ' + cpu_stress_period, shell=True,
                ...
                self.wfile.write(
                    "Processing the cpu stress with " + cpu_stress_workers + " workers spinning sqrt() for " + cpu_stress_period + " seconds!")
                ...
                with open(script_folder + "/anomaly.log", "a") as logFile:
                    logFile.write(
                        str(time.time()) + ", io, " + str(io_stress_period) + ", " + str(io_stress_workers) + "\n")

                p = sub.Popen('stress --io ' + io_stress_workers + ' --timeout ' + io_stress_period, shell=True,
                              stdout=sub.PIPE, stderr=sub.PIPE)
                ...
                # serve the HTML code to client on Google App Engine Python using webapp2
                self.wfile.write(
                    "Processing the I/O stress with " + io_stress_workers + " workers spinning rsync() for " + io_stress_period + " seconds!")
                return
                ...
                # Append stress log to anomaly.log
                with open(script_folder + "/anomaly.log", "a") as logFile:
                    logFile.write(str(time.time()) + ", mem, " + str(mem_stress_period) + ", " + str(
                        mem_stress_workers) + ", " + str(mem_size_per_worker) + "\n")

                p = sub.Popen(
                    'stress --vm ' + mem_stress_workers + ' --vm-bytes ' + mem_size_per_worker + 'M --timeout ' + mem_stress_period,
                ...
                # serve the HTML code to client on Google App Engine Python using webapp2
                self.wfile.write(
                    "Processing the memory stress with " + mem_stress_workers + " workers spinning malloc()/free() with " + mem_size_per_worker + " MB per worker for " + mem_stress_period + " seconds!")
                return
                ...
                if bwIP.lower() == 'all':
                    ...
                    bw_cmd = script_folder + '/limitOutboundPerIP.sh ' + str(bw_stress_period) + ' ' + bwIP + ' ' + str(
                        bw_capacity) + 'Kbps'
                    cmd_name = 'bw-perIP'

                # Append stress log to anomaly.log
                with open(script_folder + "/anomaly.log", "a") as logFile:
                    logFile.write(str(time.time()) + "," + cmd_name + ", " + str(bw_stress_period) + ", " + str(
                        bw_capacity) + "," + bwIP + "\n")
                ...
                # serve the HTML code to client on Google App Engine Python using webapp2
                self.wfile.write(
                    "Throttling the bandwidth with cmd " + cmd_name + " to " + bw_capacity + "kbps for " + bw_stress_period + " seconds!")
                return
                ...
                if lat_ip.lower() == 'all':
                    ...
                    add_lat_cmd = script_folder + '/addLatPerIP.sh ' + str(lat_period) + ' ' + str(lat_ip) + ' ' + str(
                        lat_val)
                print(add_lat_cmd)
                ...
                # serve the HTML code to client on Google App Engine Python using webapp2
                self.wfile.write(
                    "Add latency " + lat_val + " for packets to prefix " + lat_ip + " for " + lat_period + " seconds!")
                return
                ...
                if drop_ip.lower() == 'all':
                    ...
                    drop_pkts_cmd = script_folder + '/dropPktsPerIP.sh ' + str(drop_period) + ' ' + str(
                        drop_ip) + ' ' + str(drop_percent)

                # Append stress log to anomaly.log
                with open(script_folder + "/anomaly.log", "a") as logFile:
                    logFile.write(
                        str(time.time()) + ",dropPkts," + str(drop_period) + "," + drop_ip + "," + drop_percent + "\n")
                ...
                # serve the HTML code to client on Google App Engine Python using webapp2
                self.wfile.write(
                    "Drop " + drop_percent + " packets to IP prefix " + drop_ip + " for " + drop_period + " seconds!")
                return
